...
Original file is located at
    https://colab.research.google.com/drive/1O2wlJxpw-wliCQQjjZUcSwfra0N9k6zW

<a href="https://colab.research.google.com/github/GoodEy3/GoodEy3/blob/main/MLb.ipynb" target="_parent"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a>
"""

!pip install duckdb xgboost pandas numpy datetime joblib tqdm requests meteostat scikit-learn matplotlib pybaseball

!git clone https://huggingface.co/datasets/yubsogay/MLb_data2
...
TEAM_ABBR_MAP = {
    ...
    # Ensure all teams in BALLPARK_COORDS have a mapping if their API abbr differs
    'ARI': 'ARI', 'ATL': 'ATL', 'BAL': 'BAL', 'BOS': 'BOS', 'CIN': 'CIN',
    'CLE': 'CLE', 'COL': 'COL', 'DET': 'DET', 'HOU': 'HOU', 'LAD': 'LAD',
    'MIA': 'MIA', 'MIL': 'MIL', 'MIN': 'MIN', 'OAK': 'OAK', 'PHI': 'PHI',
    ...
    if isinstance(obj, (np.integer, np.int_)):
        ...
        if np.isnan(obj):
            ...
            logging.warning(
                ...
                f"Encountered Infinity value during JSON conversion: {obj}. Replacing with None.")
        ...
        return [convert_numpy_types(i) for i in

                obj.tolist()]  # Use tolist() for better handling
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if status_code == 401:
                ...
                logging.error(
                    ...
                    f"Request failed for {url}: Invalid API key or credentials (401).")
            ...
            if status_code == 429:
                ...
                logging.warning(
                    ...
                    f"Request failed for {url}: Rate limit likely exceeded (429).")

                # Exponential backoff for rate limiting [14]

                sleep_time = (2 ** attempt) + np.random.uniform(0,
            ...
            logging.warning(
                ...
                f"Attempt {attempt + 1}/{max_retries}: Request failed for {url} - {e}")

        except Exception as e:

            logging.error(

                f"Attempt {attempt + 1}/{max_retries}: Unexpected error during request for {url}: {e}")
        ...
        if attempt < max_retries - 1:
            ...
            logging.info(
                ...
                f"Retrying request for {url} in {sleep_time:.2f} seconds...")
...
def get_schedule_for_date(prediction_date):
    ...
    logging.info(
        ...
        f"Fetching MLB schedule for {formatted_date} from {MLB_SCHEDULE_API_URL}")
    ...
    if response is None:
        ...
        logging.error(
            ...
            f"Failed to fetch MLB schedule for {formatted_date} after multiple retries.")
    ...
    try:
        ...
        if 'dates' not in data or not data['dates']:
            ...
            logging.warning(
                ...
                f"No schedule data found in API response for {formatted_date}.")
        ...
        for game in games:
            ...
            if game_status in ['Final', 'Game Over',
                               ...
                               'Completed Early'] or 'Postponed' in detailed_status:

                logging.debug(

                    f"Skipping game {game.get('gamePk')} with status: {game_status} / {detailed_status}")
            ...
            home_probable_pitcher_id = game.get('teams', {}).get('home',

                                                                {}).get(

                'probablePitcher', {}).get('id')

            away_probable_pitcher_id = game.get('teams', {}).get('away',
            ...
            home_team_abbr_mapped = TEAM_ABBR_MAP.get(home_team_abbr,

                                                     home_team_abbr)

            away_team_abbr_mapped = TEAM_ABBR_MAP.get(away_team_abbr,
            ...
            game_info = {
                ...
                'home_team_abbr': home_team_abbr_mapped,

                # Use mapped abbreviation

                'away_team_abbr': away_team_abbr_mapped,
            ...
            if home_probable_pitcher_id: all_player_ids.add(

                home_probable_pitcher_id)

            if away_probable_pitcher_id: all_player_ids.add(
        ...
        logging.info(
            ...
            f"Found {len(scheduled_games)} scheduled (non-final/non-postponed) games for {formatted_date}.")
        ...
        logging.error(
            ...
            f"Error decoding MLB schedule JSON for {formatted_date}: {e}")
        ...
        logging.error(
            ...
            f"Unexpected error processing schedule for {formatted_date}: {e}")
...
def calculate_daily_score(df_in, is_pitcher):
    ...
    if is_pitcher:
        ...
        fip_constant = LEAGUE_AVERAGES.get('FIP_constant', 3.10) # Get from averages or use default
        # FIP = ((13 * HR) + (3 * (BB + HBP)) - (2 * K)) / IP + constant
        fip_numerator = (13 * df['home_run'] + 3 * (df['walk'] + df['hit_by_pitch']) - 2 * df['strikeout'])

        # FIX [I-01]: Handle IP = 0 to avoid division by zero. Assign a penalty FIP.
        lg_fip = LEAGUE_AVERAGES.get('lgFIP', 3.80) # Get from averages or use default
        ...
        runs_per_win_war = LEAGUE_AVERAGES.get('runs_per_win', 9.5) # Get from averages or use default
        ...
        league_woba = LEAGUE_AVERAGES.get('league_woba', 0.315)
        woba_scale = LEAGUE_AVERAGES.get('wOBA_scale', 1.25)
        runs_per_win_owar = LEAGUE_AVERAGES.get('runs_per_win', 9.5)
        ...
        if league_woba <= 0 or woba_scale <= 0 or runs_per_win_owar <= 0:
            ...
            df['daily_wrc_plus'] = np.where(
                df['PA'] > 0,
                100.0 * (df['daily_woba'] / league_woba), # Assuming no park/league adjustments needed for daily score
        ...
        # Center wRC+ around 0, scale
        scaled_wrc_plus = (df['daily_wrc_plus'] - 100.0) / 10.0
    ...
    if is_pitcher:
        ...
        keep_cols.extend(['daily_woba', 'daily_wrc_plus', 'daily_owar', 'PA', 'woba_value', 'single', 'double', 'triple', 'home_run', 'walk', 'strikeout']) # Added base hit/walk/K counts

    # Keep other columns if they are needed for feature engineering later
    ...
    keep_cols.extend(['release_speed', 'release_spin_rate', 'launch_speed', 'launch_angle', 'pfx_x', 'pfx_z', 'is_barrel']) # Added is_barrel
    ...
    # Add back any original columns (from df_in) not explicitly mentioned, if needed and present
    ...
    # Remove duplicates while preserving order (important if columns were added multiple ways)
    final_cols = list(dict.fromkeys(final_cols))

    # Filter the DataFrame to keep only the final columns
...
def load_and_process_data(player_ids=None, start_date=None, end_date=None):
    ...
    Can optionally filter by player IDs and date range. Returns two DataFrames: (batters_df, pitchers_df)
    """
    ...
    player_filter_sql = ""
    player_params = () # Use tuple for parameters
    if player_ids:
        # Ensure player IDs are native Python types
        player_ids_list = [int(pid) if isinstance(pid, (np.integer, np.int64)) else pid for pid in player_ids] # Added np.int64 check
    ...
    # Combine parameters: player params first, then date params
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            optional_stats = [
                ...
                'outs_when_up', 'is_barrel'
            ]

            # Ensure 'release_spin_rate' is used if available, map from old names if necessary
            spin_rate_col_to_use = 'release_spin_rate' # Default to new name
            if 'release_spin_rate' not in available_cols and 'spin_rate' in available_cols:
                 ...
                 spin_rate_col_to_use = 'spin_rate' # Remember which one we are actually using
                 logging.debug("Using 'spin_rate' as 'release_spin_rate' not found.")
            elif 'release_spin_rate' in available_cols and 'spin_rate' in available_cols:
                 # If both exist, prefer release_spin_rate, remove spin_rate from optionals if added
                 if 'spin_rate' in optional_stats: optional_stats.remove('spin_rate')
                 logging.debug("Both 'spin_rate' and 'release_spin_rate' found. Preferring 'release_spin_rate'.")
            ...
            has_woba_denom = 'woba_denom' in select_cols
            if 'woba_denom' in missing_essentials:
                 ...
                 select_cols = list(set(select_cols)) # Remove duplicates if events was already there
                 missing_essentials.remove('woba_denom') # Handled, remove from list
            ...
            # Construct the SELECT part of the query, renaming spin_rate if necessary
            select_cols_str_list = []
            final_df_col_names = select_cols.copy() # Track the names as they will appear in the DataFrame
            for i, col in enumerate(select_cols):
                if col == spin_rate_col_to_use and spin_rate_col_to_use == 'spin_rate':
                    select_cols_str_list.append(f'"{col}" AS "release_spin_rate"') # Rename in SQL
                    final_df_col_names[i] = 'release_spin_rate' # Update the expected final name
                    logging.debug("Mapping DB column 'spin_rate' to 'release_spin_rate' in query.")
                else:
            ...
            logging.debug(f"Expected DataFrame columns after query: {final_df_col_names}")
            ...
            if not order_cols:
                ...
                order_cols = ['game_date'] # Fallback

            # --- FIX: Generate quoted column names separately to avoid f-string backslash error ---
            quoted_order_cols = [f'"{col}"' for col in order_cols]
            order_by_clause = f"ORDER BY {', '.join(quoted_order_cols)}" # Join the already quoted names
            # --- End Fix ---

            # Construct final query and count query
            # Base query without WHERE clause (filters added dynamically)
            base_query = f"SELECT {select_cols_str} FROM pitch_data WHERE 1=1 " # Start with "WHERE 1=1" to easily append AND clauses
            query = base_query + player_filter_sql + " " + date_filter_sql + " " + order_by_clause
            count_query = f"SELECT COUNT(*) FROM pitch_data WHERE 1=1 " + player_filter_sql + " " + date_filter_sql
            ...
            try:
                ...
                # Use the combined sql_params for the count query
                ...
                if total_rows == 0:
                    ...
                    return pd.DataFrame(), pd.DataFrame() # No data to process
            ...
            # --- Process data in chunks using parameters ---
            # **** CORRECTED TRY/EXCEPT STRUCTURE ****
            try: # Start a single try block for both the query and the loop
                chunk_iterator = pd.read_sql_query(query, conn, params=sql_params, chunksize=CHUNKSIZE)

                # Keep the loop INSIDE the try block
                for batch_num, batch in enumerate(tqdm(chunk_iterator, total=n_chunks, desc="Processing DB Chunks")):
                    ...
                    # Ensure column names match expected logic (especially after potential renaming)
                    # No action needed here if final_df_col_names accurately reflects post-query names
                    ...
                    # Define essential columns for dropping NA rows
                    dropna_subset_cols = [col for col in ['game_date', 'batter', 'pitcher', 'events', 'inning_topbot'] if col in df.columns]
                    df.dropna(subset=dropna_subset_cols, inplace=True)
                    if df.empty: continue

                    # --- Calculate PA per Pitch Event ---
                    if has_woba_denom: # Check if woba_denom was successfully selected
                        # Ensure woba_denom is numeric before comparison
                        df['is_pa_outcome'] = (safe_get_numeric(df['woba_denom'], 0) == 1).astype(int)
                    elif 'events' in df.columns: # Fallback only if woba_denom missing AND events is present
                        pa_ending_events = [
                    ...
                    else: # Cannot determine PA if both woba_denom and events are missing
                        logging.warning(f"Chunk {batch_num + 1}: Cannot determine PA outcome (missing woba_denom and events). Setting PA to 0.")
                        df['is_pa_outcome'] = 0

                    # --- Calculate Outs per Event ---
                    df['outs_made'] = 0 # Initialize column
                    ...
                    if 'inning_topbot' in df.columns and 'home_team' in df.columns and 'away_team' in df.columns:
                        ...
                        logging.debug(f"Chunk {batch_num + 1}: Missing columns for opponent team calculation.")
                        df['batter_opp_team'] = None
                        df['pitcher_opp_team'] = None
                    ...
                    default_vals.update({'delta_home_win_exp': 0.0,
                                         ...
                                         # Use the correct spin rate default (already handled by LEAGUE_AVERAGES above if needed)
                                         })
                    ...
                    if 'is_barrel' not in df.columns:
                        calculate_barrel = True
                         ...
                         if df['is_barrel'].isnull().any(): # Check again if safe_get_numeric failed (might happen if column is all NaN)
                             calculate_barrel = True # Still need to calculate if filling failed
                    ...
                    if calculate_barrel:
                        if 'launch_speed' in df.columns and 'launch_angle' in df.columns:
                            ...
                            df['is_barrel'] = 0 # Create the column and set to 0
                    elif 'is_barrel' in df.columns: # Ensure existing column is integer if not recalculated
                         # If safe_get_numeric ran, it might be float; ensure int
                         df['is_barrel'] = df['is_barrel'].astype(int)
                    else: # Should not happen if calculate_barrel logic is correct, but as a fallback
                         df['is_barrel'] = 0


                    # --- Aggregation Dictionary ---
                    # Define aggregations dynamically based on available columns
                    agg_dict_base = {
                        # Counting stats
                    ...
                    # Ensure essential aggregation columns exist AFTER creating them
                    ...
                    if missing_essentials_agg:
                        # Check if the column exists in df but wasn't added to agg_dict (shouldn't happen with current logic)
                        # More likely, the column wasn't created properly earlier (e.g., PA calculation failed)
                        logging.warning(f"Chunk {batch_num + 1} missing essential keys in agg_dict ({missing_essentials_agg}). This might indicate issues creating these columns earlier. Skipping aggregation.")
                        continue # Skip aggregation for this chunk if essential components are missing

                    if not agg_dict:
                         logging.warning(f"Chunk {batch_num + 1} has no columns to aggregate. Skipping.")
                         continue

                    # --- Perform Aggregations (Inside Loop, still Inside Try) ---
                    # Use an inner try/except for robustness during aggregation/scoring
                    try:
                        # Group by game date and player ID
                        group_cols_batter = ['game_date', 'batter']
                        if 'batter' in df.columns and 'batter' in group_cols_batter: # Only aggregate if batter column exists
                        ...
                        if 'pitcher' in df.columns and 'pitcher' in group_cols_pitcher: # Only aggregate if pitcher column exists
                        ...
                        logging.error(f"Error during aggregation/scoring in chunk {batch_num + 1}: {agg_err}", exc_info=True) # Log traceback
                        # Decide if you want to continue to the next chunk or stop
                        continue # Continue to next chunk on aggregation error

                # --- End of For Loop ---

            # Now, the except block is at the correct level to handle errors
            # from EITHER read_sql_query OR the processing loop
            except sqlite3.Error as db_err: # Catch specific DB errors first if needed
                logging.error(f"Database error during query or data fetching: {db_err}", exc_info=True)
                # logging.debug(f"Failed Query: {query} | Params: {sql_params}")
                return pd.DataFrame(), pd.DataFrame() # Return empty DFs on DB error
            except Exception as e: # Catch other errors (e.g., processing errors in the loop)
                logging.error(f"An error occurred during data loading or processing: {e}", exc_info=True) # Log traceback
                # Log context if possible (which chunk caused the error)
                # Note: 'batch_num' might not be defined if the error was in read_sql_query itself
                if 'batch_num' in locals():
                    logging.debug(f"Error occurred around chunk {batch_num + 1}")
                else:
                    logging.debug(f"Error likely occurred during initial data query.")
                # Return empty DFs on general processing error
                return pd.DataFrame(), pd.DataFrame()
            # --- End of Corrected Try/Except Block ---

        # --- End of 'with sqlite3.connect...' block ---

    except sqlite3.Error as e:
        logging.error(f"Failed to connect to or read from database {DB_PATH}: {e}", exc_info=True)
        return pd.DataFrame(), pd.DataFrame()
    except Exception as e:
        logging.error(f"An unexpected error occurred outside the main processing loop: {e}", exc_info=True)
        return pd.DataFrame(), pd.DataFrame()
    ...
    if batter_chunks:
        try:
            batters_df = pd.concat(batter_chunks, ignore_index=True)
            # Optional: Sort final DataFrame
            if 'game_date' in batters_df.columns and 'batter' in batters_df.columns:
                 batters_df.sort_values(by=['batter', 'game_date'], inplace=True)
            logging.info(f"Successfully processed {len(batters_df)} batter game entries.")
        except Exception as concat_err:
            logging.error(f"Error concatenating batter chunks: {concat_err}", exc_info=True)
            batters_df = pd.DataFrame() # Return empty if concatenation fails

    if pitcher_chunks:
        try:
            pitchers_df = pd.concat(pitcher_chunks, ignore_index=True)
             # Optional: Sort final DataFrame
            if 'game_date' in pitchers_df.columns and 'pitcher' in pitchers_df.columns:
                 pitchers_df.sort_values(by=['pitcher', 'game_date'], inplace=True)
            logging.info(f"Successfully processed {len(pitchers_df)} pitcher game entries.")
        except Exception as concat_err:
            logging.error(f"Error concatenating pitcher chunks: {concat_err}", exc_info=True)
            pitchers_df = pd.DataFrame() # Return empty if concatenation fails
...
if __name__ == '__main__':
    ...
    if not os.path.exists(DB_PATH):
        ...
        # Create a simple table structure - adjust columns based on your actual needs
        conn_test.execute('''
            CREATE TABLE pitch_data (
                game_date TEXT, batter INTEGER, pitcher INTEGER, events TEXT, inning_topbot TEXT,
                home_team TEXT, away_team TEXT, game_pk INTEGER, at_bat_number INTEGER, pitch_number INTEGER,
                delta_home_win_exp REAL, woba_value REAL, woba_denom INTEGER, description TEXT,
                launch_speed REAL, launch_angle REAL, release_speed REAL, release_spin_rate REAL,
                pfx_x REAL, pfx_z REAL, is_barrel INTEGER
            )
        ''')
        # Add some dummy data
        dummy_data = [
            ('2025-04-01', 665742, 660271, 'single', 'Top', 'HOU', 'CWS', 718781, 1, 1, 0.05, 0.9, 1, 'hits line drive single', 105.0, 15.0, 95.0, 2300.0, -0.5, 1.5, 1),
            ('2025-04-01', 665742, 660271, 'strikeout', 'Top', 'HOU', 'CWS', 718781, 2, 3, -0.10, 0.0, 1, 'strikes out swinging', None, None, 96.0, 2400.0, -0.6, 1.6, 0),
            ('2025-04-01', 545361, 676606, 'field_out', 'Bot', 'HOU', 'CWS', 718781, 3, 2, -0.03, 0.0, 1, 'grounds out', 88.0, -5.0, 92.0, 2100.0, 1.2, 0.8, 0),
            ('2025-04-01', 545361, 676606, 'home_run', 'Bot', 'HOU', 'CWS', 718781, 4, 1, 0.25, 2.0, 1, 'hits home run', 110.0, 28.0, 93.0, 2150.0, 1.1, 0.9, 1),
             # Add data for another date
            ('2025-04-02', 665742, 680686, 'walk', 'Top', 'TEX', 'PHI', 718790, 1, 4, 0.08, 0.7, 1, 'walks', None, None, 88.0, 2800.0, 0.9, 0.1, 0),
            ('2025-04-02', 545361, 663395, 'double', 'Bot', 'TEX', 'PHI', 718790, 2, 2, 0.15, 1.25, 1, 'line drive double', 108.0, 19.0, 94.5, 2250.0, -0.8, 1.3, 1)
        ]
        conn_test.executemany('INSERT INTO pitch_data VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', dummy_data)
        conn_test.commit()
        conn_test.close()
        print("Dummy database created.")

    # Example call (using current date based on thought process)
    today = datetime.now()
    start_dt = datetime(today.year, 4, 1) # Example start date
    end_dt = datetime(today.year, 4, 2)   # Example end date
    print(f"\nRunning example for dates: {start_dt.date()} to {end_dt.date()}")
    batters, pitchers = load_and_process_data(start_date=start_dt, end_date=end_dt)
    # Example call without date filter:
    # batters, pitchers = load_and_process_data()
    # Example call with player filter:
    # batters, pitchers = load_and_process_data(player_ids=[665742])
    ...
    if not batters.empty:
        ...
        print("Batters DataFrame is empty.")
    ...
    if not pitchers.empty:
        ...
        print("Pitchers DataFrame is empty.")

# Key Changes Implemented:
# * Corrected try...except Scope: The main try block now starts before the pd.read_sql_query call and ends after the for loop that processes the chunks.
# * Consolidated Error Handling: The except sqlite3.Error and except Exception blocks now follow the entire query-and-process block, correctly catching errors from either the initial query or any part of the subsequent data processing within the loop.
# * Inner Try/Except for Aggregation: Added an optional, more specific try...except block inside the for loop around the groupby().agg() and calculate_daily_score() calls. This helps pinpoint errors during aggregation/scoring for a specific chunk and allows the processing to potentially continue with the next chunk if desired (currently set to continue).
# * Robust Column Handling: Added checks (if col in df.columns:) before attempting operations on columns that might be optional or missing in some chunks (like batter, pitcher, handling opponent teams, barrel calculation inputs).
# * PA Calculation Fallback: Explicitly checks if has_woba_denom: before using it, and falls back to using events only if woba_denom is missing and events is present.
# * Spin Rate Handling: Added logic to detect if release_spin_rate or the older spin_rate column exists and aliases spin_rate to release_spin_rate in the SQL query if necessary, ensuring the rest of the code consistently uses release_spin_rate.
# * Parameter Handling: Ensured SQL parameters are correctly handled as tuples.
# * Placeholders: Added placeholders for DB_PATH, CHUNKSIZE, LEAGUE_AVERAGES, safe_get_numeric, and basic logging setup, which you'll need to replace with your actual values or imports.
# * Example Usage: Included a basic if __name__ == '__main__': block with dummy database creation and an example function call.
# * Commented Out Notes: The final section explaining the key changes has been commented out using '#' to prevent the SyntaxError.
# * F-string Backslash Fix: Corrected the construction of the ORDER BY clause to avoid using backslashes within the f-string expression part.
