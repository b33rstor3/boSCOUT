# breakout_scout_v1.2.py
# Module to identify historical breakout performances from MLB data,
# selecting top N per day based on game count, and saving key underlying stats.
...
OUTPUT_TABLE_NAME = "breakout_performances_topN_stats" # <<-- Updated table name
...
def load_historical_data(start_date_str, end_date_str):
    ...
    select_cols_str = ', '.join(list(set(select_cols_sql))) # Use set to remove duplicates
...
# <<< --- START: Integrated Scoring Logic --- >>>

def calculate_single_game_score(df_in, is_pitcher):
    """
    Calculates the daily performance score (-10 to 10) and key underlying stats
    ...
    """
    # Initialize default return structure with all potential stats
    default_return = {
        'daily_score': np.nan, 'game_date': pd.NaT, 'player_id': pd.NA, 'game_pk': pd.NA,
        'daily_fip': np.nan, 'daily_war': np.nan, 'IP': np.nan,
        'daily_woba': np.nan, 'daily_wrc_plus': np.nan, 'daily_owar': np.nan, 'PA': np.nan,
        # Add aggregated stats needed for display
        'H': 0, 'R': 0, 'ER': 0, 'BB': 0, 'SO': 0, 'HR': 0, # Common stats
        'outs_recorded': 0, # Pitcher specific base stat
        'single': 0, 'double': 0, 'triple': 0, # Hitter specific base stats (HR already included)
        'SB': 0, 'CS': 0, 'RBI': 0 # Placeholder for now, need pitch data for these
    }

    if df_in is None or df_in.empty:
        return pd.DataFrame([default_return])

    if len(df_in) > 1:
         df = df_in.iloc[[0]].copy()
    else:
         df = df_in.copy()
    ...
    # --- Get Aggregated Stats (used for both scoring and display) ---
    # Use .get() with default Series to handle missing columns gracefully
    # Pitcher Base Stats
    outs_recorded = df.get('outs_recorded', pd.Series([0], index=df.index)).iloc[0]
    p_home_run = df.get('home_run', pd.Series([0], index=df.index)).iloc[0] # Renamed to avoid clash
    p_walk = df.get('walk', pd.Series([0], index=df.index)).iloc[0]
    p_hit_by_pitch = df.get('hit_by_pitch', pd.Series([0], index=df.index)).iloc[0]
    p_strikeout = df.get('strikeout', pd.Series([0], index=df.index)).iloc[0]
    # Hitter Base Stats
    PA = df.get('PA', pd.Series([0], index=df.index)).iloc[0]
    h_single = df.get('single', pd.Series([0], index=df.index)).iloc[0]
    h_double = df.get('double', pd.Series([0], index=df.index)).iloc[0]
    h_triple = df.get('triple', pd.Series([0], index=df.index)).iloc[0]
    h_home_run = df.get('home_run', pd.Series([0], index=df.index)).iloc[0]
    h_walk = df.get('walk', pd.Series([0], index=df.index)).iloc[0]
    h_hit_by_pitch = df.get('hit_by_pitch', pd.Series([0], index=df.index)).iloc[0]
    h_strikeout = df.get('strikeout', pd.Series([0], index=df.index)).iloc[0]
    # Common Base Stats
    ...
    p_home_run = safe_get_numeric(pd.Series([p_home_run])).iloc[0]
    p_walk = safe_get_numeric(pd.Series([p_walk])).iloc[0]
    p_hit_by_pitch = safe_get_numeric(pd.Series([p_hit_by_pitch])).iloc[0]
    p_strikeout = safe_get_numeric(pd.Series([p_strikeout])).iloc[0]
    PA = safe_get_numeric(pd.Series([PA])).iloc[0]
    h_single = safe_get_numeric(pd.Series([h_single])).iloc[0]
    h_double = safe_get_numeric(pd.Series([h_double])).iloc[0]
    h_triple = safe_get_numeric(pd.Series([h_triple])).iloc[0]
    h_home_run = safe_get_numeric(pd.Series([h_home_run])).iloc[0]
    h_walk = safe_get_numeric(pd.Series([h_walk])).iloc[0]
    h_hit_by_pitch = safe_get_numeric(pd.Series([h_hit_by_pitch])).iloc[0]
    h_strikeout = safe_get_numeric(pd.Series([h_strikeout])).iloc[0]
    woba_value = safe_get_numeric(pd.Series([woba_value])).iloc[0]
    daily_wpa = safe_get_numeric(pd.Series([daily_wpa])).iloc[0]

    # --- Role-Specific Calculations ---
    daily_fip, daily_war, IP = np.nan, np.nan, np.nan
    daily_woba, daily_wrc_plus, daily_owar = np.nan, np.nan, np.nan
    daily_score = np.nan # Initialize score
    # Initialize display stats
    H_disp, R_disp, ER_disp, BB_disp, SO_disp, HR_disp = 0, 0, 0, 0, 0, 0
    AB_disp, SB_disp, CS_disp, RBI_disp = 0, 0, 0, 0 # Need AB, R is base stat

    try:
        if is_pitcher:
            IP = outs_recorded / 3.0
            fip_constant = LEAGUE_AVERAGES['FIP_constant']
            fip_numerator = (13 * p_home_run + 3 * (p_walk + p_hit_by_pitch) - 2 * p_strikeout)
            ...
            # Assign pitcher display stats (Need H, R, ER from aggregation if possible)
            # These might require joining back with game logs or more complex aggregation
            H_disp = df.get('H_allowed', pd.Series([0], index=df.index)).iloc[0] # Placeholder name
            R_disp = df.get('R_allowed', pd.Series([0], index=df.index)).iloc[0] # Placeholder name
            ER_disp = df.get('ER_allowed', pd.Series([0], index=df.index)).iloc[0] # Placeholder name
            BB_disp = int(p_walk)
            SO_disp = int(p_strikeout)
            HR_disp = int(p_home_run)
            ...
            # Assign hitter display stats
            H_disp = int(h_single + h_double + h_triple + h_home_run)
            # R, RBI, SB, CS need more detailed data than usually aggregated here
            R_disp = df.get('R_scored', pd.Series([0], index=df.index)).iloc[0] # Placeholder name
            RBI_disp = df.get('RBI', pd.Series([0], index=df.index)).iloc[0] # Placeholder name
            SB_disp = df.get('SB', pd.Series([0], index=df.index)).iloc[0] # Placeholder name
            CS_disp = df.get('CS', pd.Series([0], index=df.index)).iloc[0] # Placeholder name
            BB_disp = int(h_walk)
            SO_disp = int(h_strikeout)
            HR_disp = int(h_home_run)
            # AB needs to be aggregated if not already present
            AB_disp = df.get('AB', pd.Series([0], index=df.index)).iloc[0] # Placeholder name


    except Exception as e:
         logging.error(f"Error calculating score/stats for player {player_id} on {game_date}: {e}")
    ...
    result_data = {
        'daily_score': final_score, 'game_date': game_date, 'player_id': player_id, 'game_pk': game_pk,
        ...
        'H': H_disp, 'R': R_disp, 'ER': ER_disp, 'BB': BB_disp, 'SO': SO_disp, 'HR': HR_disp, # Common display stats
        'outs_recorded': outs_recorded if is_pitcher else 0, # Pitcher specific base stat
        'single': h_single if not is_pitcher else 0, # Hitter specific base stats
        'double': h_double if not is_pitcher else 0,
        'triple': h_triple if not is_pitcher else 0,
        'AB': AB_disp if not is_pitcher else 0, # Hitter specific
        'SB': SB_disp if not is_pitcher else 0, # Hitter specific (placeholder)
        'CS': CS_disp if not is_pitcher else 0, # Hitter specific (placeholder)
        'RBI': RBI_disp if not is_pitcher else 0 # Hitter specific (placeholder)
    }

    # Ensure integer types where appropriate
    int_cols_result = ['player_id', 'H', 'R', 'ER', 'BB', 'SO', 'HR', 'outs_recorded', 'single', 'double', 'triple', 'AB', 'SB', 'CS', 'RBI', 'PA']
    for col in int_cols_result:
         if col in result_data:
              result_data[col] = int(safe_get_numeric(pd.Series([result_data[col]]), 0).iloc[0])
...
def aggregate_and_calculate_scores(df):
    ...
    Includes game_pk and key stats in the output.
    """
    ...
    # Define aggregation operations including base stats for display
    agg_ops = {
        ...
        # Add more base stats if needed (e.g., H, R, ER for pitchers - requires more complex logic usually)
    }
    ...
    if 'batter' in df.columns:
        try:
            ...
            if not batter_agg.empty:
                 results = []
                 # Use tqdm on the groupby object directly
                 for name, group in tqdm(batter_agg.groupby(['game_date', 'player_id', 'game_pk'], observed=True), desc="Scoring Batters", total=batter_agg.groupby(['game_date', 'player_id', 'game_pk']).ngroups):
    ...
    if 'pitcher' in df.columns:
        try:
            ...
            if not pitcher_agg.empty:
                 results = []
                 # Use tqdm on the groupby object directly
                 for name, group in tqdm(pitcher_agg.groupby(['game_date', 'player_id', 'game_pk'], observed=True), desc="Scoring Pitchers", total=pitcher_agg.groupby(['game_date', 'player_id', 'game_pk']).ngroups):
...
def identify_breakouts(player_perf_df):
    ...
    def select_top_n_for_date(date_group):
        ...
        n_to_select = TOP_N_HIGH_GAMES if num_games >= GAMES_THRESHOLD else TOP_N_LOW_GAMES
...
def save_breakouts(breakout_df):
    ...
    try:
        with sqlite3.connect(OUTPUT_DB_PATH) as conn:
            ...
            # Ensure player_id and game_pk are standard int before saving if needed, handle potential NA
            for id_col in ['player_id', 'game_pk']:
                 if id_col in df_to_save.columns:
                      df_to_save[id_col] = pd.to_numeric(df_to_save[id_col], errors='coerce').astype('Int64')
            ...
            for col in df_to_save.select_dtypes(include=[np.number]).columns:
                 if df_to_save[col].isnull().any():
                      # Check for nullable integer types specifically
                      if pd.api.types.is_integer_dtype(df_to_save[col]):
                           # Convert to float first to handle <NA>, then fillna with np.nan
...
if __name__ == "__main__":
    ...
    if not raw_pitch_data.empty:
        ...
        del raw_pitch_data # Free memory
        gc.collect()
